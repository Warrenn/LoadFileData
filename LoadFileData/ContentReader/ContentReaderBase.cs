...
namespace LoadFileData.ContentReader
{
    public abstract class ContentReaderBase : IContentReader
    {
        ...
        public virtual IEnumerable<IDictionary<string, object>> RowData(Stream fileStream)
        {
            ...
            foreach (var data in ReadRowData(fileStream)
                .Select(rowData => rowData as object[] ?? rowData.ToArray())
                .Where(data => !data.All(o => string.IsNullOrEmpty(string.Format("{0}", o)))))
            {
                if (headers.Length == 0)
                {
                    var index = 0;
